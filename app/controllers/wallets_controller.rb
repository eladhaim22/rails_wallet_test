  ...
  def update
      respond_to do |format|
        begin
          ...
          rescue Exceptions::CreditCardUpdateError
            @wallet.errors.add(:base,'cannot update credit cards')
            format.html { render :edit }
            format.json { render json: @wallet.errors, status: :unprocessable_entity} 
          end
      end
  ...
  private
    ...
    def wallet_params
      params.require(:wallet).permit(:name,{
        credit_cards_attributes:[:number,:CCV,:expiration_date,:id, '_destroy']})
    end
end
